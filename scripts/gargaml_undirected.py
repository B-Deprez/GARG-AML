...
# results/<dataset>_GARGAML_undirected.csv and read back by gargaml_tree.py.
...
# Name suffixes: "_bank<b>"/"_banktop<k>" a single-institution view, "_res<r>"
# a Louvain resolution, "_nolouvain" no reduction, "_hubs<k>" hub removal
# instead of Louvain (see graph_processing.parse_resolution). The name is used
# verbatim in the output path, so each entry OVERWRITES its own file in place.
# LI-Large and the no-Louvain arms are the expensive runs. Stage 2
# (gargaml_tree.py) skips a dataset with missing measures, so an interrupted
# run still leaves a usable results/.
