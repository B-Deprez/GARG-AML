...
# results/<dataset>_GARGAML_directed.csv and read back by gargaml_tree.py.
...
# src/utils/graph_processing.parse_resolution). The name is used verbatim in
# the output path, so each entry OVERWRITES that file in place. LI-Large and
# the no-Louvain arms are the expensive runs. gargaml_tree.py skips a dataset
# whose measures are missing, so a trimmed or interrupted run still leaves a
# usable results/.
